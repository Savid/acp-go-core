...
// turns, activities, or actions. Its encoded shape is the family fixture
// battery's: eight members, every one present.
type State struct {
	...
	// Closed reports that the addressed session's close completed: no later event
	// is possible from it, and no further incarnation of it exists.
...
// Settled reports whether nothing the stream knows about is live: the
// foreground is idle and every activity and action is terminal.
