...
| `acpcore/usage` and provider subpackages | Bounded provider account readers returning `wire.AccountUsageResponse`, and verification that native credentials and routes remain stable during a read; callers supply credentials and an optional HTTP transport. Readers never acquire credentials or launch a harness. |
